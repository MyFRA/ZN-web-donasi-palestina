...
interface CartContextInterface {
    ...
    reloadCarts: (cb?: () => void) => void;
    addToCart: (id: number, successCb: () => void, errCb: () => void) => void;
}

export const CartContext = createContext<CartContextInterface>({ carts: [], reloadCarts: (cb?: () => void) => {}, addToCart: (id: number, successCb: () => void, errCb: () => void) => {} });

const CartContextProvider = ({ children }: { children: React.ReactNode }) => {
    const [carts, setCarts] = useState<Array<CartInterface>>([]);

    const reloadCarts = (cb?: () => void) => {
        ...
        if (!storageCarts) {
            ...
            Promise.all(
                cartsTemp.map(async (cartTemp: CartInterface) => {
    ...
    return <CartContext.Provider value={{ carts, reloadCarts, addToCart }}>{children}</CartContext.Provider>;
};
